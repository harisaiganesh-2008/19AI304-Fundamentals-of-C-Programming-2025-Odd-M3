...
int main() {
    ...
    // Step 4: Input decimal number
    printf("Enter a decimal number: ");
    scanf("%d", &num);

    // Step 6: Conversion logic
    int temp = num; // keep original number for display
    while (temp > 0) {
        rem = temp % 2;        // remainder (0 or 1)
        binary[i] = rem;       // store in array
        i++;                   // increment index
        temp = temp / 2;       // update number
    }
...
int main()
{
    ...
    for (i = 0; i < m; i++)
    {
        ...
        if (min == max &&
            ...
            pos[0][1] == pos[1][1])
        {
            printf("\nSaddle Point found at position (%d, %d)",
                   pos[0][0], pos[0][1]);
            printf("\nSaddle Point value: %d\n", min);
            found = 1;
        }
    }
